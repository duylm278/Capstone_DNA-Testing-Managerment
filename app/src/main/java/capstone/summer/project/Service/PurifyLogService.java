...
public class PurifyLogService {
    static final String TAG = "PurifyLogService";
    static final ObjectMapper om = new ObjectMapper().configure(MapperFeature.ACCEPT_CASE_INSENSITIVE_PROPERTIES, true);
    static final String URL_API = ConstantManager.HOST+"api/" + ConstantManager.API.GET_PURIFYLOG+"/PurifyProcess";
    static final Gson gson = new Gson();

    public static void getPurifyLogByID(Context context, int  id, final VolleyCallback callback) {
        ...
        StringRequest objectRequest = new StringRequest(
                ...
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        try {
                            List<PurifyLog> purifyLogs= om.readValue(response,new TypeReference<List<PurifyLog>>(){});
                            callback.onSuccess(purifyLogs);
                        } catch (Exception ex) {
                            ex.printStackTrace();
